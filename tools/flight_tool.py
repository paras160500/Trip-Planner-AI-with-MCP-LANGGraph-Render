...
DEFAULT_ORIGIN = os.getenv("DEFAULT_ORIGIN_DATA" , "BOM")
...
def clean_text(text: str) -> str:
    """
        Removing the stopwords and cleaning the text
        for better data reviewing and responsing 
        Args:
            text(str) : user query
        Returns:
            cleaned String 
...
def country_name_to_code(text : str):
    """
        From provided country name find the code
        related to that country.
        Args:
            text(str) : user question
        Returns:
            give the coutnry code
    """
    ...
    # Direct Country name inside longer text
...
def airport_country_matches(airport : dict , country_code : str) -> bool:
    """
        Does the airport and the country code match or not
        Args:
            airport(dict) : Airport in dict
...
def get_best_airport_for_country(country_code : str):
    """
        Based on the country code get the best airport
        of that particular country pass in the argument.
        Args:
...
def resolve_location_to_iata(location : str):
    """
        Convertes country/city/airport/IATA into IATA code.
        Exmaples:
        JAPAN -> NRT
        TOKYO -> NRT
    """
